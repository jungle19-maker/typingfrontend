...
const Report = () => {
    ...
    const processData = useCallback((data) => {
        ...
        // Filter by Language? For now, let's show separate stats or a toggle?
        // Let's separate "Hindi" vs "English" stats in the process logic
        const hindiData = data.filter(d => d.language === 'hindi');
        const englishData = data.filter(d => d.language !== 'hindi'); // Retroactive compatibility

        // Default to English stats for main cards for now, or aggregate?
        // Requirement: "Language-wise performance"
        // Let's set Global Stats to be "All" but add specific breakdowns.
        ...
        // Hindi Specific
        const hindiWpm = hindiData.length > 0 ? Math.round(hindiData.reduce((acc, curr) => acc + curr.wpm, 0) / hindiData.length) : 0;
        const hindiAcc = hindiData.length > 0 ? Math.round(hindiData.reduce((acc, curr) => acc + curr.accuracy, 0) / hindiData.length) : 0;
        ...
        setStats({
            ...
            hindiWpm,
            hindiAcc,
            englishCount: englishData.length,
            hindiCount: hindiData.length
        });
        ...
        const groupedByMode = data.reduce((acc, curr) => {
            ...
            const key = `${mode}-${lang}`;
            if (!acc[key]) acc[key] = { name: mode, language: lang, wpmSum: 0, accSum: 0, count: 0 };
            acc[key].wpmSum += curr.wpm;
            acc[key].accSum += curr.accuracy;
            acc[key].count += 1;
            return acc;
        }, {});
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            const normalizedExams = examRes.data.map(exam => ({
                ...exam,
                gameType: 'Exam: ' + (exam.exam?.title || 'Unknown'),
                mistakeCount: exam.errorCount,
                date: exam.completedAt,
                language: exam.exam?.language || 'english' // examinations usually have a defined language
            }));

            const combinedData = [...practiceRes.data, ...normalizedExams].sort((a, b) => new Date(b.date) - new Date(a.date));
        ...
        <div className="app-container p-6 md:p-10 max-w-7xl mx-auto space-y-8">
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                        <p className="text-gray-400">Welcome back, {user?.username || 'Guest'}.</p>
                        {user?.subscription?.planName && (
                            <span className={`px-2 py-0.5 rounded text-xs font-bold uppercase border ${['pro', 'premium'].includes(user.subscription.planName)
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white/5 border border-white/10 p-6 rounded-2xl backdrop-blur-sm">
                    ...
                    <FeatureGuard feature="weeklyReports" showUpgrade={true}>
                        {trendData.length > 0 ? (
                ...
                <div className="bg-white/5 border border-white/10 p-6 rounded-2xl backdrop-blur-sm flex flex-col justify-between">
                    <div>
                        ...
                        <FeatureGuard feature="detailedAnalytics" showUpgrade={true}>
